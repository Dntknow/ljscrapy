...
class MysqlClient(object):
    ...
    def creater():
        connect = pymysql.connect(host=self.host, user=self.user, password=self.password, port=self.port)
        cursor = connect.cursor(self.cursor)  # 返回dict类型数据， 默认为Cursor是列表数据
        cursor.execute('CREATE DATABASE IF NOT EXISTS %s DEFAULT CHARSET utf8 COLLATE utf8_general_ci;' % db)
        connect.commit()
...
def in_sql(data):
    url = "118.190.101.204"
    sql = MysqlClient(url, 3306, 'root', '12345', 'lianjia')
    time.sleep(0.2)
    ...
    values = values % tuple(data.values())
    cmd = 'insert into {table}({keys}) values({values})'.format(table=table, keys=keys, values=values)
    sql.execute_cmd(cmd)
    #print('='*5, sql)
